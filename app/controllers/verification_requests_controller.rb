class VerificationRequestsController < ApplicationController
  ...
  def index
    @verification_requests = @building.verification_requests.page(params[:page]).per(10)
  end
  ...
  def create
    ...
    if @verification_request.save
      redirect_to building_verification_request_path(@building, @verification_request), notice: 'Verification request was successfully created.'
    else
      render :new
    end
  end
  ...
  def destroy
    ...
    redirect_to building_verification_requests_url, notice: 'Verification request was successfully destroyed.'
  end
